...
결제 환불은 예약 크레딧과 무관하게 전액 차감한다. 이미 사용했거나 예약한 금액 때문에 잔액이 음수가 되면 부채를 그대로 표시하고 추가 AI 예약을 차단한다. 이후 정산/취소는 해당 예약액만 반환하므로 환불로 크레딧이 새로 생기지 않는다. Stripe 구독 items와 invoice lines의 기간도 반영한다.

결제 인식과 구독 권한의 이벤트 시각은 별도로 관리한다. 확인된 구독 상태가 있으면 invoice.paid는 크레딧만 지급하고 활성 상태를 변경하지 않는다. 같은 초의 상충하는 구독 이벤트는 취소 상태를 우선하며 이후 시각의 명시적 구독 이벤트만 상태를 갱신한다.

대화 작업은 요청 시점의 지원 공고와 선택 문서 버전을 고정한다. 문서만 선택하면 당시 최신 버전을 사용하고 버전이 없으면 오류를 반환한다. 문서의 근거 참조도 지원 범위 승인을 확인하여 모델 입력에 포함한다. 결과 첨부는 서버가 검증한 DOCUMENT_VERSION/EVIDENCE 참조로 생성하며 모델이 ID나 실행 동작을 지정하지 않는다. 대화는 문서를 수정하거나 승인을 생성·소비하지 않는다.

면접 companySources는 사용자가 확인한 HTTPS 출처·원문·조회 시각을 최대 10개 보관한다. 서버가 URL을 자동 조회했다고 표시하지 않는다. 로컬 및 AI 준비 결과의 research는 저장 원문을 정확히 발췌하고 USER_PROVIDED 상태로 표시한다. AI 작업은 요청 당시 출처를 영속화하며 모델이 만든 회사 주장이나 URL을 채택하지 않는다. 자료가 없으면 research는 비어 있어 입력이 필요하다.
